  ...
  return (
    <div className="w-full min-h-screen text-white overflow-x-hidden">
      ...
      <nav className="flex justify-between items-center px-8 py-6 max-w-7xl mx-auto" style={{ zIndex: 100 }}>
        <Link to="/" className="text-2xl font-bold flex items-center gap-2" style={{ color: 'var(--primary)' }}>
        ...
        <div className="flex gap-8 items-center text-sm font-semibold tracking-wide">
          ...
          <a href="#training" className="hover:text-indigo-400 transition">TRAINING</a>
          <Link to="/login" className="btn-primary" style={{ padding: '0.6rem 1.5rem', borderRadius: '9999px' }}>
            Login Platform
          </Link>
        </div>
      </nav>
      ...
      <section className="px-8 py-20 flex flex-col items-center justify-center text-center animate-fade-in" style={{ minHeight: '80vh' }}>
        <h1 className="text-6xl font-bold mb-6 max-w-4xl tracking-tight leading-tight" style={{ textShadow: '0 4px 20px rgba(0,0,0,0.5)' }}>
          Welcome to <span style={{ color: 'var(--accent)' }}>SkillGraph</span>
        </h1>
        ...
        <div className="flex gap-4">
          <Link to="/login" className="btn-primary flex items-center gap-2" style={{ padding: '1rem 2rem', fontSize: '1.1rem' }}>
            Get Started <ArrowRight size={20} />
          </Link>
          <a href="#services" className="flex items-center gap-2" style={{ padding: '1rem 2rem', fontSize: '1.1rem', backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: '12px', border: '1px solid var(--card-border)' }}>
            Our Services
          </a>
        </div>
      </section>
      ...
      {/* Stats/About Section */}
      <section id="about" className="px-8 py-20" style={{ backgroundColor: 'rgba(0,0,0,0.2)' }}>
      ...
      <section id="training" className="px-8 py-24 max-w-7xl mx-auto relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="p-8 rounded-2xl" style={{ backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid var(--card-border)' }}>
            <h3 className="text-2xl font-bold mb-4 flex items-center gap-3"><Cloud className="text-indigo-400" /> Salesforce Training</h3>
          ...
          <div className="p-8 rounded-2xl" style={{ backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid var(--card-border)' }}>
            <h3 className="text-2xl font-bold mb-4 flex items-center gap-3"><Database className="text-green-400" /> Data Science & Engineering</h3>
            <ul className="text-gray space-y-3 mb-6">
          ...
          <div className="p-8 rounded-2xl" style={{ backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid var(--card-border)' }}>
            <h3 className="text-2xl font-bold mb-4 flex items-center gap-3"><Code className="text-purple-400" /> DevOps & Software Cloud</h3>
            <ul className="text-gray space-y-3 mb-6">
          ...
          <div className="p-8 rounded-2xl flex flex-col justify-center items-center text-center" style={{ backgroundColor: 'rgba(99, 102, 241, 0.05)', border: '1px solid rgba(99, 102, 241, 0.3)' }}>
            <BookOpen size={40} className="mb-4 text-indigo-400" />
            <h3 className="text-2xl font-bold mb-2">Automated Skill Profiling</h3>
            <p className="text-gray mb-6">Log into our AI-driven SkillGraph portal to map your technical skills dynamically and calculate your career roadmap instantly.</p>
            <Link to="/login" className="btn-primary inline-flex">Go To App <ArrowRight size={18} className="ml-2" /></Link>
          </div>
        </div>
      </section>

      {/* Footer */}
      <footer className="py-8 text-center text-gray border-t border-gray-800" style={{ backgroundColor: 'var(--bg-dark)' }}>
